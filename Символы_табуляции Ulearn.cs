...
namespace Символы_табуляции
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ReplaceIncorrectSeparators(@"Страна Население Дата %
...
Франция - 68859599; 01.01.2018, 0.88%");
        ...
        public static string ReplaceIncorrectSeparators(string text)
        {
            ...
            {
                var sNew = s.Split(stringSeparators, StringSplitOptions.None);
                lst.Add(string.Join("\t", sNew.ToArray()));
            }
            return string.Join("\n", lst.ToArray());
        }
    }
